...
# Barcode_col_name: the column name of meta which is the same with colnames(rna)/cell barcodes in expression matrix
getPseudobulkList <- function(mat, celltypes, samples, 
                              pooling_function = c("sum", "mean"), 
                              meta = NULL, 
                              barcode_col_name = NULL, 
...
# Takes nested list barcodes (Celltype X Sample) from getCellSampleBarcodesList and returns nested pseudobulk DGElists (Celltype X sample)
...
# can filter such that only pools with a sufficient number of cells is maintained
...
poolExpressionSampleBarcodeList <- function(mat, sample_barcode_list, pooling_function = c("sum", "mean")){
  pool_mat <- lapply(sample_barcode_list, function(barcodes){
    if(pooling_function == "sum"){
      Matrix::rowSums(mat[, barcodes, drop = FALSE]) # do not drops any unused levels, e.g. fpr single column data
    }else if(pooling_function == "mean"){
      Matrix::rowMeans(mat[, barcodes, drop = FALSE])
    }else{
