...
### MISC TCR analysis ######################################
############################################################
# Generates Figure 5C for CD4 TCR TRBV11-2 usage
...
# ggsave(p, device = "pdf", filename = "output/TCR_allTcells_TRBVgene_T40.pdf", width = 25, height = 17.5)
...
p <- ggplot(trbv112, aes(x=days_since_symptom_onset, y=ratio))+
...
ggsave(p, device = "pdf", filename = "output/TCR_allTcells_TRBV112_time.pdf", width = 4, height = 3)
...
ggsave(p, device = "pdf", filename = "plots/figures/TCR_allTcells_TRBV112_T40.pdf", width = 4, height = 3)
...
ggsave(p, device = "pdf", filename = "TCR/plots/TCR_CD4_TRBVgene_T40.pdf", width = 25, height = 17.5)

# time effect Figure 5C
...
p <- ggplot(cd4_trbv112, aes(x=days_since_symptom_onset, y=ratio))+
  ...
  xlab("Days_since_hospitalization")
ggsave(p, device = "pdf", filename = "output/TCR_CD4Tcells_TRBV112_time.pdf", width = 4, height = 3)
