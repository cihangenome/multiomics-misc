...
merge.B.cite.cor$padj <- p.adjust(merge.B.cite.cor$cor.pval,method="BH")
...
p <- ggplot(merge.B.cite.cor.plot, aes(x=reorder(marker, abs(cor.r)), y=cor.r)) +
  ...
  xlab("CITE_Markers")+
  theme_bw()
ggsave(p, device = "pdf", filename = "output/plots/BCR_MemB_mut_corr_cite.pdf", width = 4, height = 4)
...
p <- ggplot(merge.B.cite.cor.plot, aes(x=reorder(marker, abs(cor.r)), y=cor.r)) +
  ...
  theme_bw()
p
ggsave(p, device = "pdf", filename = "output/plots/BCR_PB_mut_corr_cite.pdf", width = 4, height = 4)
